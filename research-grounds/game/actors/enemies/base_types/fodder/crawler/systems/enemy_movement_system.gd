...
func _init(body_ref: CharacterBody2D, data_ref: EnemyFodderData, detect_ref: DetectionSystem) -> void:
	body = body_ref
	data = data_ref
	detection =  detect_ref
...
func chase(_delta: float) -> void:
	if not detection.has_player():
		_stop()
		return
	var target_pos := detection.get_player_position()
